...
export default class Hand {
  ...
  hasCard(card: Card): boolean {
    return this.cards.some((c) => c.suit === card.suit && c.face === card.face);
  }

  removeCard(card: Card): void {
    const index = this.cards.findIndex((c) => c.suit === card.suit && c.face === card.face);
  ...
  playCard(card: Card, leadSuit?: SuitType): void {
    ...
    const isLegal = playable.some((c) => c.suit === card.suit && c.face === card.face);
