...
test('cant bid more than 6 tricks', function () {
  assert.throws(
    ...
    "Shouldn't allow bidding more than 6 tricks",
  );
});
...
test('cant bid less than 0 tricks', function () {
  assert.throws(
    ...
    /Invalid number of tricks/,
    "Shouldn't allow bidding less than 0 tricks",
  );
});
...
test('loner bid must be for 6 tricks', function () {
  assert.throws(
    ...
    /Loner bid must be for 6 tricks/,
    'Loner bid should require 6 tricks',
  );
});
...
test('cant bid a suited contract without a suit', function () {
  assert.throws(
    ...
    /Suited bid must have a suit type/,
    "Shouldn't allow bidding a suited contract without a suit",
  );
});
...
test('cant bid a suit on a high contract', function () {
  assert.throws(
    ...
    /Non-suited bids cannot have a suit type/,
    "Shouldn't allow bidding a suit on a high contract",
  );
});
...
test('cant bid a suit on a low contract', function () {
  assert.throws(
    ...
    /Non-suited bids cannot have a suit type/,
    "Shouldn't allow bidding a suit on a low contract",
  );
});
...
test('beats should return true for stronger contract type', function () {
  ...
  assert.strictEqual(
    strongerBid.beats(weakerBid),
    true,
    'Stronger contract type should beat weaker contract type',
  );
});
